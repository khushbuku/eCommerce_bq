  ...
  derived_table: {
    explore_source: order_items {
      ...
      derived_column: previous_day_check {
        sql: lead(total_sale_price,1,0) over(order by returned_date)  ;;
      }
      derived_column: previous_2day_check {
        sql: lead(total_sale_price,2,0) over(order by returned_date)  ;;
      }
      derived_column: previous_3day_check {
        sql: lead(total_sale_price,3,0) over(order by returned_date)  ;;
      }
